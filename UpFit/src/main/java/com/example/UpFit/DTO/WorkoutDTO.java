...
//   [B] edit by smsong : 보조(assisted) / 원본 lbs(origLbs) 추가.
//     @AllArgsConstructor 의 인자 순서 = 필드 선언 순서이므로, 엔티티/DTO 양쪽의
//     선언 위치를 동일하게 맞춰야 생성자 호출부가 어긋나지 않는다.
//     선언 순서: ... bodyweight, assisted, origLbs, sortOrder
//   [E] edit by smsong
@NoArgsConstructor
@AllArgsConstructor
@Getter
...
public class WorkoutDTO {
    ...
    private Double origLbs;      // [B][E] edit by smsong : 사용자가 lbs 로 입력했을 때의 원래 값. null = kg 입력/맨몸
    ...
    public static WorkoutDTO entityToDto(WorkoutEntity e) {
        return new WorkoutDTO(
                ...
                e.getOrigLbs(),       // [B][E] edit by smsong
                e.getSortOrder());
    }

    public WorkoutEntity dtoToEntity() {
        return new WorkoutEntity(id, userId, sessionId, exercise, weight, reps, sets, memo, bodyweight, assisted, origLbs, sortOrder);
    }
}
// [E] edit by smsong
